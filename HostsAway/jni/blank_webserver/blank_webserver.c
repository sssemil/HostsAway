...
int main(void) {
  ...
  __android_log_print(ANDROID_LOG_INFO, THIS_FILE, "HostsAway Native Webserver: starting");
  while (s_sig_num == 0) {
    mg_mgr_poll(&mgr, 1000);
  }

  mg_mgr_free(&mgr);
  __android_log_print(ANDROID_LOG_INFO, THIS_FILE, "HostsAway Native Webserver: exited on signal %d", s_sig_num);
  return EXIT_SUCCESS;
}
